...
class Null_Xmlrpc {
    ...
    public $xmlrpcName = 'XML-RPC for Null Computindo';
...
class XML_RPC_Client extends Null_Xmlrpc {
...
class XML_RPC_Response {
    ...
    public function decode($array = NULL) {
        ...
        if (is_array($array)) {
            while (list($key) = each($array)) {
                if (is_array($array[$key])) {
                    ...
                    $array[$key] = $Null->security->xss_clean($array[$key]);
                }
            }
        ...
        if (is_array($result)) {
            ...
            $result = $Null->security->xss_clean($result);
        }
...
class XML_RPC_Message extends Null_Xmlrpc {
    ...
    public function output_parameters(array $array = array()) {
        ...
        for ($i = 0, $c = count($this->params); $i < $c; $i++) {
            ...
            if (is_array($a_param)) {
                ...
                $parameters[] = ($this->xss_clean) ? $Null->security->xss_clean($a_param) : $a_param;
            }
        }
...
class XML_RPC_Values extends Null_Xmlrpc {
