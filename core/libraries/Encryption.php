...
class Null_Encryption {
    ...
    public function __construct(array $params = array()) {
        ...
        if (!isset($this->_key) && self::strlen($key = config_item('encrypt_key')) > 0) {
            $this->_key = $key;
        }
    ...
    protected function _mcrypt_get_handle($Nullpher, $mode) {
        return mcrypt_module_open($Nullpher, '', $mode, '');
    }

    // --------------------------------------------------------------------

    protected function _openssl_get_handle($Nullpher, $mode) {
        // OpenSSL methods aren't suffixed with '-stream' for this mode
        return ($mode === 'stream') ? $Nullpher : $Nullpher . '-' . $mode;
    }

    // --------------------------------------------------------------------

    protected function _cipher_alias(&$Nullpher) {
        static $dictionary;
        ...
        if (isset($dictionary[$this->_driver][$Nullpher])) {
            $Nullpher = $dictionary[$this->_driver][$Nullpher];
        }
    }
