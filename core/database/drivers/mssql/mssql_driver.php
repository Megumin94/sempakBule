...
class Null_DB_mssql_driver extends Null_DB {
    ...
    protected function _delete($table) {
        if ($this->qb_limit) {
            return 'WITH null_delete AS (SELECT TOP ' . $this->qb_limit . ' * FROM ' . $table . $this->_compile_wh('qb_where') . ') DELETE FROM null_delete';
        }
    ...
    protected function _limit($sql) {
        ...
        if (version_compare($this->version(), '9', '>=') && $this->qb_offset && !empty($this->qb_orderby)) {
            ...
            // Get the fields to select from our subquery, so that we can avoid Null_rownum appearing in the actual results
            ...
            return 'SELECT ' . $select . " FROM (\n\n"
                    . preg_replace('/^(SELECT( DISTINCT)?)/i', '\\1 ROW_NUMBER() OVER(' . trim($orderby) . ') AS ' . $this->escape_identifiers('Null_rownum') . ', ', $sql)
                    . "\n\n) " . $this->escape_identifiers('Null_subquery')
                    . "\nWHERE " . $this->escape_identifiers('Null_rownum') . ' BETWEEN ' . ($this->qb_offset + 1) . ' AND ' . $limit;
        }
