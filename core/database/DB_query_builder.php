...
abstract class Null_DB_query_builder extends Null_DB_driver {
    ...
    public function count_all_results($table = '', $reset = TRUE) {
        ...
        $result = ($this->qb_distinct === TRUE) ? $this->query($this->_count_string . $this->protect_identifiers('numrows') . "\nFROM (\n" . $this->_compile_select() . "\n) Null_count_all_results") : $this->query($this->_compile_select($this->_count_string . $this->protect_identifiers('numrows')));
    ...
    protected function _compile_wh($qb_key) {
        if (count($this->$qb_key) > 0) {
            for ($i = 0, $c = count($this->$qb_key); $i < $c; $i++) {
                ...
                for ($Null = 0, $cc = count($conditions); $Null < $cc; $Null++) {
                    if (($op = $this->_get_operator($conditions[$Null])) === FALSE
                            OR ! preg_match('/^(\(?)(.*)(' . preg_quote($op, '/') . ')\s*(.*(?<!\)))?(\)?)$/i', $conditions[$Null], $matches)) {
                        continue;
                    }
                    ...
                    $conditions[$Null] = $matches[1] . $this->protect_identifiers(trim($matches[2]))
